...
class ConfigEditor extends VBox {
    ...
    ConfigEditor(Path data_path, Map<String,Path> configuration) {
        dataPath = data_path
        ...
        Button saveConfigBtn = new Button("Save preference")
        saveConfigBtn.setOnAction(new EventHandler<ActionEvent>() {
            @Override public void handle(ActionEvent e) {
            	// SAVE
            	ConfigurationManager.write(config, dataPath.resolve("config.xml"))
            }
        })
    ...
    VBox createFieldNode(String name, String title, boolean isFolder) {
        ...
        if (path != null)
    	   //txtField.setText(dataPath.relativize(path).toString())
           txtField.setText(path.toString())
        ...
        btn.setOnAction(new EventHandler<ActionEvent>() {
            @Override public void handle(ActionEvent e) {
                ...
                if (newPath != null) {
                    println newPath
                    // txtField.setText(dataPath.relativize(newPath).toString())
                    txtField.setText(newPath.toString())
                    config[name] = newPath
